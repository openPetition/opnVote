...
export async function checkEthCall(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        try {
          ...
          if (isTimeout && attempt === 1) {
            logger.warn(
              `[EthCall] RPC call timeout, retrying for user: ${signatureData.struct.user}`,
            )
            continue
          } else {
            throw error
      ...
      logger.info(
        `[EthCall] RPC call completed in ${rpcDuration}ms for user: ${signatureData.struct.user}`,
      )

      if (result.toLowerCase() !== '0x') {
        logger.warn(
          `[EthCall] Unexpected return value from transaction simulation: ${result} for user: ${signatureData.struct.user} and target: ${signatureData.struct.target}`,
        )
        return res.status(400).json({
          data: null,
      ...
      logger.info(
        `[EthCall] Successfully validated transaction in ${totalDuration}ms for user: ${signatureData.struct.user}`,
      )
      next()
    } catch (callError) {
      const rpcDuration = Date.now() - startTime
      logger.error(
        `[EthCall] Transaction simulation failed after ${rpcDuration}ms for user: ${signatureData.struct.user}. Error: ${callError}`,
      )
      return res.status(400).json({
        data: null,
