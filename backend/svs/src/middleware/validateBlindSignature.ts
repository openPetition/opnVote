 ...
 */
export async function validateBlindSignature(
  req: Request,
  res: Response,
  next: NextFunction,
  ...
  try {
    ...
    const registerPubKey: RSAParams | null = await ElectionService.getElectionRegisterPublicKey(
      electionId,
    )
    if (!registerPubKey) {
    ...
    const isValidSignature: Boolean = verifyUnblindedSignature(
      unblindedSignature,
