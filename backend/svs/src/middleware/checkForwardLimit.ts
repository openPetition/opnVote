...
async function incrementForwardCountWithUpsertAndCheck(
...
export async function checkForwardLimit(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    logger.info(
      `[ForwardLimit] Starting forward limit check for address: ${senderAddress}, MAX_FORWARDS: ${MAX_FORWARDS}`,
    )
    ...
    if (!result.allowed) {
      logger.warn(
        `[ForwardLimit] Forward limit exceeded for address: ${senderAddress}. Limit: ${MAX_FORWARDS}, Attempted count: ${
          result.finalCount
        } after ${Date.now() - dbStartTime}ms`,
      )
      ...
      logger.info(
        `[ForwardLimit] Forward limit check passed for address: ${senderAddress}. Final count: ${
          result.finalCount
        } after ${Date.now() - dbStartTime}ms`,
      )
    }
