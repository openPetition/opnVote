 ...
 */
export async function checkForExistingSvsSignature(
  req: Request,
  res: Response,
  next: NextFunction,
  ...
  logger.info(
    `[ExistingSVS] Starting check for existing SVS signature at ${new Date().toISOString()}`,
  )
  ...
  try {
    ...
    const normalizedElectionTokenHex: string = normalizeHexString(
      votingTransaction.unblindedElectionToken.hexString.toLowerCase(),
    )
    const normalizedUnblindedSignatureHex: string = normalizeHexString(
    ...
    if (partialMatch) {
      logger.error(
        `[ExistingSVS] Found inconsistent data for election ${electionId}: ${partialMatch}`,
      )
    ...
    const totalDuration = Date.now() - startTime
    logger.error(
      `[ExistingSVS] Error checking for existing SVS signature after ${totalDuration}ms: ${error}`,
    )
