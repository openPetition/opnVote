...
export async function processPendingRequests(gelatoRelay: GelatoRelay, repository: Repository<GelatoQueueEntity>) {
    try {
        ...
        for (const request of pendingRequests) {
            ...
            try {
                ...
                logger.info(`[Task ID: ${relayResponse.taskId}] Request ${request.requestHash} submitted successfully after ${request.retryCount} retries.`);
            } catch (error) {
                logger.error(`[Task ID: ${request.gelatoTaskId || 'N/A'}] Error processing Gelato request (Request Hash: ${request.requestHash}):`, error);
                ...
                if (request.retryCount < MAX_RETRIES) {
                    ...
                    logger.info(`[Task ID: ${request.gelatoTaskId || 'N/A'}] Request ${request.requestHash} marked for retry (${request.retryCount}/${MAX_RETRIES}).`);
                } else {
                    ...
                    logger.info(`[Task ID: ${request.gelatoTaskId || 'N/A'}] Request ${request.requestHash} failed after maximum retries.`);
                }
                await repository.save(request);
            }
        }
...
export async function updateTaskStates(gelatoRelay: GelatoRelay, repository: Repository<GelatoQueueEntity>) {
    try {
        ...
        for (const task of submittedTasks) {
            if (task.gelatoTaskId) {
                try {
                    const statusResponse: TransactionStatusResponse | undefined = await gelatoRelay.getTaskStatus(task.gelatoTaskId);
                        ...
                        case TaskState.Cancelled:
                            if (task.retryCount < MAX_RETRIES) {
                                ...
                                task.failureReason = truncateErrorMessage(
                                    `Max retries (${MAX_RETRIES}) reached. Last error: ${statusResponse.lastCheckMessage || "Unknown error"}`,
                                    MAX_FAILURE_REASON_LENGTH
                                );
                                logger.info(`[Task ID: ${task.gelatoTaskId}] Marked for retry (${task.retryCount}/${MAX_RETRIES}).`);
                            } else {
                                task.status = GelatoQueueStatus.FAILED;
                                task.failureReason = truncateErrorMessage(`Max retries (${MAX_RETRIES}) reached. Last error: ${statusResponse.lastCheckMessage || "Unknown error"}`, MAX_FAILURE_REASON_LENGTH);
