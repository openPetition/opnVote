...
import checkForExistingBlindedSignature, {
  unauthenticatedCheckForExistingBlindSignature,
} from '../middleware/checkExistingBlindSignatures'
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const { voterId, electionId } = req.user!
      const authHeader = req.headers.authorization!
      const blindedToken = req.body.token as Token

      logger.debug(`Processing registration for voter ${voterId} in election ${electionId}`)
      startTime = Date.now()
      ...
      try {
        ...
        if (e.code === 'ER_DUP_ENTRY') {
          ...
          const existing = await dataSource
            .getRepository(BlindedSignature)
            .findOneBy({ voterId, electionId })
          if (existing!.blindedToken.toLowerCase() === blindedToken.hexString.toLowerCase()) {
            return res.json({
              data: {
                message: 'Existing blinded signature found.',
                blindedSignature: existing!.blindedSignature.toLowerCase(),
              },
              error: null,
            })
          }
          return res
            .status(400)
            .json({ data: null, error: 'Already registered with different token' })
        }
        logger.error(e)
