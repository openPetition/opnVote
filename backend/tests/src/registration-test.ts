...
async function makeRegistrationRequest(userID: number): Promise<number | null> {
  try {
    ...
    if (!resData?.data?.blindedSignature) {
      console.error(
        `UserID ${userID}: Invalid response structure or missing blindedSignature. Response:`,
    ...
    if (dataLevelKeyCount >= 1 && resData.data.blindedSignature) {
      if (topLevelKeyCount > 2 || dataLevelKeyCount > 1) {
        console.warn(
          `UserID ${userID}: Received blindedSignature, but response contained unexpected data. Response:`,
          JSON.stringify(resData),
        )
      }
      return userID
      ...
      console.error(
        `UserID ${userID}: BlindedSignature missing or invalid structure. Response:`,
        JSON.stringify(resData),
      )
      return null
    }
...
async function verifyOnChainRegistrations(stats: TestStats): Promise<void> {
  ...
  console.log(
    `Starting on-chain verification for ${initialPendingCount} UserIDs. Timeout: ${
      config.onChainCheckTimeoutMs / 1000
    }s`,
  )
  ...
  while (Date.now() < verificationEndTime && stats.pendingUserIDs.size > 0) {
    try {
      ...
      if (foundIDsThisCheck.length > 0) {
        console.log(
          `[On-Chain Check] Verified ${foundIDsThisCheck.length} more UserIDs. Remaining: ${
            stats.pendingUserIDs.size
          }. Verified IDs: ${foundIDsThisCheck.join(', ')}`,
        )
      } else {
        console.log(
          `[On-Chain Check] No new UserIDs verified. Remaining: ${stats.pendingUserIDs.size}.`,
        )
      }
  ...
  if (stats.pendingUserIDs.size > 0) {
    ...
    console.log(
      `Failed to verify ${stats.failedOnChain} UserIDs on-chain within the timeout: ${remainingIDs}`,
    )
  ...
  console.log(
    `On-chain verification phase finished in ${duration.toFixed(2)}s. Verified: ${
      stats.successfulOnChain
    }, Timed Out/Failed: ${stats.failedOnChain}`,
  )
}
...
function printResults(stats: TestStats): void {
  ...
  if (stats.pendingUserIDs.size > 0) {
    console.warn(
      `Warning: ${stats.pendingUserIDs.size} UserIDs might still be pending verification if test ended prematurely.`,
    )
  }
  ...
  if (stats.successfulOnChain < stats.successfulApi) {
    console.error(
      `Test FAILED: Only ${stats.successfulOnChain}/${stats.successfulApi} registrations verified on-chain`,
    )
    process.exit(1)
  }
