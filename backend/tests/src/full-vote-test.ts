...
const TEST_RUN_OFFSET = TEST_RUN_ID * 100_000_000 // guarantees 100M space between different test cases
const baseUserId = modifiedTimestamp + TEST_RUN_OFFSET

if (TEST_RUN_ID > 20) {
  throw new Error(`TEST_RUN_ID too large. Max allowed is 20`)
}
...
const RegisterPublic: RSAParams = {
  e: BigInt('0x10001'),
  N: BigInt(
    '',
  ),
...
const coordinatorKeyPublic: EncryptionKey = {
  hexString:
    '',
...
async function createAuthToken(electionID: number, userID: number): Promise<string> {
  try {
    ...
    if (process.env.AP_PRIVATE_KEY) {
      ...
      throw new Error(
        'No private key available - set PRIVATE_KEY environment variable or provide a key file',
      )
    }
...
async function sendRegistrationRequest(
  voter: Voter,
): Promise<{ userID: number | null; blindedSignature: string | null }> {
  try {
    ...
    if (!resData?.data?.blindedSignature) {
      console.error(
        `UserID ${voter.userID}: Invalid response structure or missing blindedSignature. Response:`,
    ...
    if (dataLevelKeyCount >= 1 && resData.data.blindedSignature) {
      if (topLevelKeyCount > 2 || dataLevelKeyCount > 1) {
        console.warn(
          `UserID ${voter.userID}: Received blindedSignature, but response contained unexpected data. Response:`,
          JSON.stringify(resData),
        )
      }
      ...
      console.error(
        `UserID ${voter.userID}: BlindedSignature missing or invalid structure. Response:`,
        JSON.stringify(resData),
      )
      return { userID: null, blindedSignature: null }
    }
    ...
    if (typeof error === 'object' && error !== null && 'response' in error) {
      ...
      if (axiosError.response) {
        console.error(
          `Status: ${axiosError.response.status}, Response: ${JSON.stringify(
...
async function sendSvsSignRequest(
  votingTransaction: VotingTransaction,
  voterSignature: EthSignature,
): Promise<EthSignature | null> {
  try {
...
async function sendSvsForwardRequest(signatureData: any): Promise<string | null> {
  try {
    const response = await axios.post<any>(config.svsForwardUrl, signatureData, {
      headers: {
        'Content-Type': 'application/json',
      },
    })
...
async function checkGelatoTaskStatus(taskId: string): Promise<GelatoTaskStatus | null> {
  try {
    const response = await axios.get<GelatoTaskStatus>(
...
async function verifyTransactionOnChain(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      if (receipt && receipt.status === 1) {
        ...
        console.error(
          `Transaction ${transactionHash} reverted (attempt ${attempt + 1}/${maxAttempts}).`,
        )
        return false
      }
      console.log(
        `Transaction ${transactionHash} not yet confirmed (attempt ${
          attempt + 1
        }/${maxAttempts}). Receipt: ${JSON.stringify(receipt)}`,
      )
    } catch (error) {
      console.error(
        `Error fetching receipt for ${transactionHash} (attempt ${attempt + 1}/${maxAttempts}):`,
        error,
      )
    }
  ...
  console.error(
    `Transaction verification failed for hash ${transactionHash} after ${maxAttempts} attempts.`,
  )
  return false
}

async function waitForGelatoTask(
  taskId: string,
  options: { timeoutMs: number; intervalMs: number },
): Promise<GelatoTaskStatus | null> {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    console.log(
      `Task ${taskId} is still pending (${taskState}). Attempt ${attempt + 1}/${maxAttempts}`,
    )
  ...
  for (const taskInfo of taskInfos) {
    try {
      const gelatoStatus = await waitForGelatoTask(taskInfo.taskId, {
        timeoutMs: config.onChainCheckTimeoutMs,
        intervalMs: config.onChainCheckIntervalMs,
      })
      if (gelatoStatus?.task?.taskState === 'ExecSuccess') {
        ...
        if (txSuccess) {
          ...
          console.error(
            `Transaction verification failed for voter ${taskInfo.voterId}, txHash: ${gelatoStatus.task.transactionHash}`,
          )
        }
      } else {
        stats.failedGelatoVerify++
        console.error(
          `Gelato task verification failed for voter ${taskInfo.voterId}, taskId: ${taskInfo.taskId}`,
        )
      }
    } catch (error) {
...
export async function runLoadTest(
  ...
  console.log(
    `On-chain check interval: ${config.onChainCheckIntervalMs / 1000}s, Timeout: ${
      config.onChainCheckTimeoutMs / 1000 / 60
    }min`,
  )
  ...
  for (let i = 0; i < batches; i++) {
    ...
    for (let j = 0; j < currentBatchSize; j++) {
      ...
      const { masterToken, masterR } = generateMasterTokenAndMasterR()
      const unblindedElectionToken: Token = deriveElectionUnblindedToken(
        config.electionID,
        masterToken,
      )
      const electionR: R = deriveElectionR(
        config.electionID,
        masterR,
        unblindedElectionToken,
        config.registerKeyPublic,
      )
      const blindedElectionToken: Token = blindToken(
        unblindedElectionToken,
        electionR,
        config.registerKeyPublic,
      )
    ...
    results.forEach((result, index) => {
      ...
      if (
        result.userID !== null &&
        result.userID === originalUserID &&
        result.blindedSignature !== null
      ) {
        stats.successfulApi++
        ...
        for (const voter of voters) {
          if (voter.userID === originalUserID) {
            ...
            voter.voterCredentials = createVoterCredentials(
              unblindedSignature,
              voter.unblindedElectionToken!,
              voter.masterToken,
              config.electionID,
            )
    ...
    console.log(
      `Completed batch ${i + 1}/${batches} - API Success: ${stats.successfulApi}, API Failed: ${
        stats.failedApi
      }, Pending On-Chain: ${stats.pendingUserIDs.size}`,
    )
  ...
  console.log(
    `API Registration phase complete. Total successful API requests: ${stats.successfulApi}/${config.count}`,
  )
  ...
  for (const voter of voters) {
    try {
      if (voter.voterCredentials === null) {
        console.log(
          `Voter ${voter.userID} has no voter credentials. Registration failed. Skipping SVS Signing phase.`,
        )
        continue
      }
      ...
      const encryptedVotesAES = await encryptVotes(
        votes,
        voter.voterCredentials.encryptionKey,
        EncryptionType.AES,
      )
      const encryptedVotesRSA = await encryptVotes(
        votes,
        config.coordinatorKeyPublic,
        EncryptionType.RSA,
      )
      voter.votingTransaction = createVotingTransactionWithoutSVSSignature(
      ...
      // SVS sign request
      const svsSignature: EthSignature | null = await sendSvsSignRequest(
  ...
  printResults(stats, shouldExit)
}

function printResults(stats: TestStats, shouldExit: boolean): void {
  ...
  if (
    stats.successfulApi === config.count &&
  ...
  if (stats.successfulSvsSign + stats.failedSvsSign > 0) {
    const svsSignSuccessRate = (
      (stats.successfulSvsSign / (stats.successfulSvsSign + stats.failedSvsSign)) *
      100
    ).toFixed(2)
  ...
  console.log(
    `Attempted Gelato Verifications: ${stats.successfulGelatoVerify + stats.failedGelatoVerify}`,
  )
  ...
  if (stats.successfulGelatoVerify + stats.failedGelatoVerify > 0) {
    const gelatoSuccessRate = (
      (stats.successfulGelatoVerify / (stats.successfulGelatoVerify + stats.failedGelatoVerify)) *
      100
    ).toFixed(2)
  ...
  console.log(
    `Attempted Transaction Verifications: ${stats.successfulTxVerify + stats.failedTxVerify}`,
  )
  ...
  if (stats.successfulTxVerify + stats.failedTxVerify > 0) {
    const txSuccessRate = (
      (stats.successfulTxVerify / (stats.successfulTxVerify + stats.failedTxVerify)) *
      100
    ).toFixed(2)
  ...
  if (stats.successfulApi < config.count) {
    console.error(
      `Test FAILED: Only ${stats.successfulApi}/${config.count} API registrations succeeded`,
    )
    if (shouldExit) {
      process.exit(1)
    } else {
  ...
  if (stats.successfulSvsForward < stats.successfulSvsSign) {
    console.error(
      `Test FAILED: Only ${stats.successfulSvsForward}/${stats.successfulSvsSign} SVS forwardings succeeded`,
    )
    if (shouldExit) {
      process.exit(1)
    } else {
  ...
  if (stats.successfulGelatoVerify < stats.successfulSvsForward) {
    console.error(
      `Test FAILED: Only ${stats.successfulGelatoVerify}/${stats.successfulSvsForward} Gelato verifications succeeded`,
    )
    if (shouldExit) {
      process.exit(1)
    } else {
  ...
  if (stats.successfulTxVerify < stats.successfulGelatoVerify) {
    console.error(
      `Test FAILED: Only ${stats.successfulTxVerify}/${stats.successfulGelatoVerify} transaction verifications succeeded`,
    )
    if (shouldExit) {
      process.exit(1)
    } else {
