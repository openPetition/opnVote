 ...
 * @param {number} limit - Number of recent authorizations to fetch (default: 10).
 * @return {Promise<RecentAuthorizationResponse>} Resolves to an array of authorization entries,
 *         each containing apId and arrays of voterIds.
 *         Results are ordered by blockTimestamp in descending order.
 */
export async function fetchRecentAuthorizations(
  electionId: string,
  limit: number = 10,
): Promise<RecentAuthorizationResponse> {
  const query = gql`
    query CheckRecentVoterAuthorizations($electionId: String!, $first: Int!) {
      votersAuthorizeds(
